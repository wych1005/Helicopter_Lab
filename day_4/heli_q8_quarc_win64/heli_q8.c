 ...
 * C source code generated on : Fri Oct 11 14:04:45 2019
 *
...
void heli_q8_output0(void)             /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    /* '<S2>:1:11' */
    tmp[0] = 1.0;
    tmp[3] = sin(heli_q8_B.Sum1) * tan(heli_q8_B.Sum);
    tmp[6] = cos(heli_q8_B.Sum1) * tan(heli_q8_B.Sum);
    tmp[1] = 0.0;
    tmp[4] = cos(heli_q8_B.Sum1);
    tmp[7] = -sin(heli_q8_B.Sum1);
    tmp[2] = 0.0;
    tmp[5] = sin(heli_q8_B.Sum1) / cos(heli_q8_B.Sum);
    tmp[8] = cos(heli_q8_B.Sum1) / cos(heli_q8_B.Sum);

    /* Gain: '<S4>/Gain1' incorporates:
     *  MATLAB Function: '<Root>/Gyro vector to [pitch rate, elevation rate, travle rate]'
     */
    for (i = 0; i < 3; i++) {
      tmp_0[i] = heli_q8_P.Gain1_Gain[i + 6] * rtb_Switch[5] +
        (heli_q8_P.Gain1_Gain[i + 3] * rtb_Switch[4] + heli_q8_P.Gain1_Gain[i] *
         rtb_Switch[3]);
    }
    ...
    for (i = 0; i < 3; i++) {
      ...
      heli_q8_B.TmpSignalConversionAtToWorkspac[i] = heli_q8_B.euler_rates[i];
     ...
     *  Product: '<S9>/tg_p'
     */
    heli_q8_B.pp = atan(rtb_TmpSignalConversionAtSFunct[1] /
     ...
     *  Product: '<S9>/tg_e'
     */
    heli_q8_B.ee = atan(rtb_TmpSignalConversionAtSFunct[0] / rtb_DeadZoney);

    /* SignalConversion: '<Root>/TmpSignal ConversionAtTo WorkspaceInport1' */
    heli_q8_B.TmpSignalConversionAtToWorkspac[3] = heli_q8_B.pp;
    heli_q8_B.TmpSignalConversionAtToWorkspac[4] = heli_q8_B.ee;
    ...
    heli_q8_B.Gain[0] += heli_q8_P.Fi[0] * heli_q8_B.Joystick_gain_x;
    heli_q8_B.Gain[0] += heli_q8_P.Fi[2] * heli_q8_B.Joystick_gain_y;
    heli_q8_B.Gain[1] = 0.0;
    heli_q8_B.Gain[1] += heli_q8_P.Fi[1] * heli_q8_B.Joystick_gain_x;
    heli_q8_B.Gain[1] += heli_q8_P.Fi[3] * heli_q8_B.Joystick_gain_y;
  }
  ...
  for (i = 0; i < 2; i++) {
    rtb_Ki[i] = 0.0;
    for (i_0 = 0; i_0 < 5; i_0++) {
      rtb_Ki[i] += heli_q8_P.Ki[(i_0 << 1) + i] * tmp_1[i_0];
    }
  }
  ...
  /* Sum: '<Root>/Sum1' */
  rtb_Backgain = heli_q8_B.Gain[1] - rtb_Ki[1];

  /* Sum: '<Root>/Sum2' incorporates:
   *  Sum: '<Root>/Sum'
   */
  rtb_Frontgain = (heli_q8_B.Gain[0] - rtb_Ki[0]) + heli_q8_B.Constant1;
  if (rtmIsMajorTimeStep(heli_q8_M)) {
  }
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    /* Sum: '<S8>/Sum3' */
    heli_q8_B.Sum3 = heli_q8_B.Sum1 - heli_q8_B.Joystick_gain_x;
  }

  /* Sum: '<S8>/Sum4' */
  heli_q8_B.Sum4 = heli_q8_B.ElevationTransferFcn - heli_q8_B.Joystick_gain_y;
  if (rtmIsMajorTimeStep(heli_q8_M)) {
  }
}
...
void heli_q8_initialize(void)
{
  ...
  heli_q8_X.Integrator_CSTATE_i = heli_q8_P.Integrator_IC_i;
}
...
RT_MODEL_heli_q8_T *heli_q8(void)
{
  ...
  /* External mode info */
  heli_q8_M->Sizes.checksums[0] = (1763387274U);
  heli_q8_M->Sizes.checksums[1] = (535261308U);
  heli_q8_M->Sizes.checksums[2] = (4286381348U);
  heli_q8_M->Sizes.checksums[3] = (498358026U);
  ...
  heli_q8_M->Sizes.numBlocks = (81);   /* Number of blocks */
  heli_q8_M->Sizes.numBlockIO = (25);  /* Number of block outputs */
  heli_q8_M->Sizes.numBlockPrms = (437);/* Sum of parameter "widths" */
  return heli_q8_M;
}
