 ...
 * C source code generated on : Fri Oct 11 13:02:46 2019
 *
...
void heli_q8_output0(void)             /* Sample time: [0.0s, 0.0s] */
{
  ...
  /* SignalConversion: '<S6>/TmpSignal ConversionAtKiInport1' incorporates:
  ...
  for (i = 0; i < 2; i++) {
    rtb_Ki[i] = 0.0;
    for (i_0 = 0; i_0 < 5; i_0++) {
      rtb_Ki[i] += heli_q8_P.Ki[(i_0 << 1) + i] * tmp[i_0];
    }
  }
  ...
  /* Limited  Integrator  */
  if (heli_q8_X.Integrator_CSTATE_g >= heli_q8_P.Integrator_UpperSat) {
    heli_q8_X.Integrator_CSTATE_g = heli_q8_P.Integrator_UpperSat;
  } else {
    if (heli_q8_X.Integrator_CSTATE_g <= heli_q8_P.Integrator_LowerSat) {
      heli_q8_X.Integrator_CSTATE_g = heli_q8_P.Integrator_LowerSat;
    }
  }
...
void heli_q8_derivatives(void)
{
  ...
  lsat = (heli_q8_X.Integrator_CSTATE_g <= heli_q8_P.Integrator_LowerSat);
  usat = (heli_q8_X.Integrator_CSTATE_g >= heli_q8_P.Integrator_UpperSat);
  if (((!lsat) && (!usat)) || (lsat && (heli_q8_B.K_ei > 0.0)) || (usat &&
       (heli_q8_B.K_ei < 0.0))) {
    _rtXdot->Integrator_CSTATE_g = heli_q8_B.K_ei;
  } else {
    /* in saturation */
...
void heli_q8_initialize(void)
{
  ...
  heli_q8_X.Integrator_CSTATE_g = heli_q8_P.Integrator_IC_l;
...
RT_MODEL_heli_q8_T *heli_q8(void)
{
  ...
  /* External mode info */
  heli_q8_M->Sizes.checksums[0] = (4146446241U);
  heli_q8_M->Sizes.checksums[1] = (4169524855U);
  heli_q8_M->Sizes.checksums[2] = (1100366913U);
  heli_q8_M->Sizes.checksums[3] = (1831123923U);
